...
static uint8_t size = 0; //número de bytes do scandoce
  ...
  */
  ...
  if(! (util_sys_inb(STAT_REG, &stat))){
    ...
    if( stat & OBF ) {
      ...
      isTwoByte(&wait, &size);
      uint8_t *scanCode;
      scanCode = processCode(scanByte, size);
      return scanCode;
    }  
  }
  else{
...
uint8_t *processCode(uint8_t byte, uint8_t size){
  ...
  return scanCode; //return to a pointer -> memory leak????? 
}
